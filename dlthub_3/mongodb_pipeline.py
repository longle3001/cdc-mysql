# grep -r "ReplicatedMergeTree" /Users/longle/.pyenv/versions/3.10.13/lib/python3.10/site-packages/dlt/destinations
...
def load_select_collection_db(pipeline: Pipeline = None) -> LoadInfo:
    ...
    if pipeline is None:
        ...
        pipeline = dlt.pipeline(
            pipeline_name="local_mongo_clickhouse",
            destination='clickhouse',
            dataset_name="clickhouse_mongousers_3_2",
        )
    # Configure the source to load a few select collections incrementally
    mflix = mongodb(
        # incremental=dlt.sources.incremental("date")
    ).with_resources("mongousers_3")

    # Run the pipeline. The merge write disposition merges existing rows in the destination by primary key
    info = pipeline.run(mflix, write_disposition="append")
